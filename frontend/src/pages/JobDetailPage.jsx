...
const JobDetailPage = ({ id }) => {
  ...
  async function handleGetJobDetails() {
    try {
      const result = await getJobs(jobid);
    ...
    <main className="w-full mt-16 min-h-screen flex flex-col items-center justify-start px-4">
      <div className="w-full max-w-4xl p-4 flex flex-col items-center gap-4 my-10 border border-(--border) shadow-md rounded-2xl">
        ...
        <div className="w-full flex flex-col items-start gap-1">
          ...
          <p className="text-sm font-medium">{jobDetail.companyname}</p>

          {/* location */}
          <div className="w-full flex items-center justify-start gap-2 text-sm">
            <h2 className="font-semibold">Location:</h2>
            <p className="font-medium text-(--text)">{jobDetail.location}</p>
          </div>

          {/* experience */}
          <div className="w-full flex items-center justify-start gap-2 text-sm">
            <h2 className="font-semibold">Experience:</h2>
            <p className="font-medium text-(--text)">{jobDetail.experience}</p>
          </div>
          ...
          <div className="w-full flex items-center justify-start gap-2 text-sm">
            ...
            <p className="font-medium text-(--text)">
              {jobDetail.salary ? jobDetail.salary : "Not Mentioned"}
            </p>
          </div>
        </div>
          ...
          {jobDetail.jobdescription?.length === 1 ? (
            ...
            <ul className="w-full list-disc list-inside">
              {jobDetail.jobdescription?.map((item, key) => (
                <li key={key} className="text-sm text-(--text) font-medium">
                  {item}
                </li>
              ))}
            </ul>
        ...
        {/* skills */}
        <div className="w-full flex flex-col items-start gap-1">
          <h2 className="text-lg text-(--heading) font-semibold">Skills:</h2>
        ...
        {/* Perks */}
        <div className="w-full flex flex-col items-start gap-1">
          <h2 className="text-lg text-(--heading) font-semibold">Perks:</h2>
